...
<head>
  ...
  <script>
    document.addEventListener('click', function(e) {{
      ...
      if (!a) return;
      var name = '';
      var box = a.closest('.card-item, .detail-page, tr');
      if (box) {{
        var t = box.querySelector('h1, h3, td a strong');
        if (t) name = t.textContent.trim();
      }}
      if (!name) name = (a.textContent || '').trim();
      var dest = '';
      try {{ dest = new URL(a.href).hostname; }} catch (err) {{ dest = a.href; }}
...
<article class="detail-page">
  <div class="container container-narrow">
    ...
    <p class="review-text">{c['review']}</p>"""
        # 編集部の実体験メモ（実際に保有・利用しているカードのみ。創作は絶対にしない）
        if c.get("owner_review"):
            html += f"""
    <div class="owner-review">
      <h2>💬 編集部の実体験メモ</h2>
      <p class="owner-review-badge">※編集責任者が実際に保有・利用しているカードです。体験にもとづかない感想は掲載しません。</p>
      <p class="review-text">{c['owner_review']}</p>
    </div>"""
        html += f"""
...
def build_legal_pages(data):
    ...
    <h2>サイト開設の経緯</h2><p class="review-text">編集責任者はかつてクレジットカード会社で、カードの発行・審査・ポイントプログラム・加盟店開拓の実務に携わっていました。その経験から強く感じていたのは、「カードは発行する側の理屈で語られることが多く、使う側が本当に得をするための情報は意外と届いていない」ということです。還元率の条件が細かい注記に隠れていたり、リボ払いの仕組みが十分に理解されないまま使われていたり——業界の内側を知っているからこそ見える「もったいなさ」がたくさんありました。{site['name']}は、その経験を使う側の味方として活かすために開設したメディアです。発行側の仕組みを知る立場から、誇張なく、正確に、生活者の目線でカード選びの情報をお届けします。</p>
    ...
    <h2>編集部で実際に保有・利用しているカード</h2>
    <p class="review-text">編集責任者が現在、自身で保有し日常的に利用しているカードは以下のとおりです。当サイトの「編集部の実体験メモ」は、これらのカードの実際の利用経験にもとづいて記載しています。</p>
    <ul class="policy-list">
      <li>三井住友カード プラチナプリファード</li>
      <li>Olive フレキシブルペイ（ゴールドランク）</li>
      <li>ANA JCB ワイドゴールドカード</li>
      <li>dカード GOLD</li>
      <li>アメリカン・エキスプレス・グリーン・カード</li>
    </ul>
    <h2>監修・口コミについて（透明性のための注記）</h2><p class="review-text">当サイトは、実在しない監修者名や、根拠のない口コミ・体験談を掲載しないことを編集方針としています。掲載しているのは、公式情報をもとにした編集部独自の比較・解説と、編集部が実際に保有・利用しているカードに限った実体験です。体験にもとづかない感想や、外部からの転載口コミは掲載しません。</p>
